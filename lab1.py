...
class Solution:
    ...
    def plot_histogram_top_x_popular_items(self, x:int) -> None:
        ...
        df=pd.DataFrame.from_dict(letter_counter,orient='index',columns=['key'])
        df=df.sort_values(['key'], ascending=False).head(5)
        ax = df.plot.bar(y='key', rot=0, title="Most popular items")
        ax.set_xlabel('Items')
